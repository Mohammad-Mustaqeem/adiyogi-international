  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
        {discount > 0 && (
          <span className="badge bg-red-500 text-white text-[9px] sm:text-[10px]">{discount}% OFF</span>
        )}
        {product.stock === 0 && (
          <span className="badge bg-gray-500 text-white text-[9px] sm:text-[10px]">Out of Stock</span>
        )}
      </div>
      ...
      <div className="product-img-wrap aspect-square bg-gray-50 flex-shrink-0 overflow-hidden">
        {!imgErr && product.images?.[0] ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center p-4">
            ...
            <p className="text-[9px] sm:text-xs text-gray-300 mt-2 font-mono text-center break-all">{product.itemCode}</p>
          </div>
        )}
        {/* View detail hint */}
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1">
        <p className="text-[9px] sm:text-[10px] font-mono text-gray-400 mb-1">{product.itemCode}</p>
        ...
        <div className="mt-2 bg-navy-50 rounded-lg px-2 py-1.5 border border-navy-100">
          <p className="text-[10px] text-navy-700 font-semibold">1 PAC = {conversion} NOS</p>
        </div>
        ...
        {/* Qty + Add */}
        {product.stock !== 0 ? (
          <div className="flex items-center gap-1.5 sm:gap-2 mt-3" onClick={e => e.stopPropagation()}>
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden flex-shrink-0">
              <button onClick={e => { e.stopPropagation(); setQty(q => Math.max(1, q - 1)); }}
                className="px-2 py-1.5 sm:px-3 sm:py-2 text-navy-700 hover:bg-navy-50 font-bold text-sm transition-colors">−</button>
              <span className="px-2 text-xs sm:text-sm font-semibold min-w-[1.5rem] text-center">{qty}</span>
          ...
          <button disabled className="w-full mt-3 py-2 bg-gray-100 text-gray-400 rounded-lg text-xs sm:text-sm font-semibold" onClick={e => e.stopPropagation()}>
            Out of Stock
          </button>
        )}
      </div>

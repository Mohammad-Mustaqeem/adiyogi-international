...
export default function OrderSuccessPage() {
  ...
  const autoSent   = state?.autoSent; // { admin, customer, waReady }
  ...
  // Auto-trigger PDF download immediately after page loads
  useEffect(() => {
    if (!pdfUrl || !order) return;
    // Use a hidden iframe so the download happens silently without opening a new tab
    const timer = setTimeout(() => {
      const iframe = document.createElement('iframe');
      iframe.style.display = 'none';
      iframe.src = pdfUrl;
      document.body.appendChild(iframe);
      // Also trigger via anchor for browsers that block iframes for downloads
  ...
  const statusSteps = [
    ...
    { icon: '📱', label: waConnected ? 'WhatsApp sent automatically' : 'WhatsApp links ready', done: step >= 3 },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-navy-900 via-navy-800 to-navy-700 pt-20 pb-12 px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {[...Array(16)].map((_, i) => (
          <div key={i} className="absolute w-1.5 h-1.5 rounded-full animate-bounce opacity-30"
            style={{
              left: `${5 + i * 6}%`, top: `${10 + (i % 5) * 18}%`,
              backgroundColor: ['#C9A84C','#fff','#6B9BD2','#86efac'][i % 4],
              animationDelay: `${i * 0.15}s`, animationDuration: `${1.2 + (i % 4) * 0.4}s`,
            }} />
        ))}
      </div>
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-navy-300 text-sm mt-1">
            Order <span className="font-mono font-bold text-champagne-400">{order.orderId}</span>
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 mb-6 border border-white/20">
          ...
          <div className="space-y-3">
            {statusSteps.map((s, i) => (
              <div key={i} className={`flex items-center gap-3 transition-all duration-500 ${s.done ? 'opacity-100' : 'opacity-30'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0 transition-all duration-500 ${
                  s.done ? 'bg-green-400 text-white shadow-lg shadow-green-400/30' : 'bg-white/20'
                }`}>
                  {s.done ? '✓' : s.icon}
                </div>
                <span className={`text-sm font-medium ${s.done ? 'text-white' : 'text-white/50'}`}>{s.label}</span>
                {s.done && (
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
                ...
                <div className="text-center sm:text-left flex-1">
                  <p className="font-display font-bold text-white text-sm sm:text-base">
                    {pdfReady ? '✅ Invoice PDF Downloaded!' : '⏳ Downloading Invoice PDF...'}
                  </p>
                  <p className="text-champagne-100 text-xs sm:text-sm">
                    {pdfReady ? 'Check your downloads folder.' : 'Your invoice is being downloaded automatically.'}
                  </p>
                </div>
                <a
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className={`rounded-2xl p-4 mb-5 border-2 ${
              waConnected ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'
            }`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className={`font-bold text-sm ${waConnected ? 'text-green-800' : 'text-blue-800'}`}>
                    {waConnected ? '✅ WhatsApp Sent Automatically' : '📱 WhatsApp Notifications'}
                  </p>
                  <p className={`text-xs ${waConnected ? 'text-green-600' : 'text-blue-600'}`}>
                    {waConnected
              ...
              {waConnected && (
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-white rounded-xl px-3 py-2 text-center">
                    <p className="text-xs text-gray-500 mb-0.5">Admin Notified</p>
                    <p className={`text-sm font-bold ${adminSent ? 'text-green-600' : 'text-orange-500'}`}>
                      {adminSent ? '✓ Sent' : '⚠ Failed'}
                    </p>
                  </div>
                  <div className="bg-white rounded-xl px-3 py-2 text-center">
                    <p className="text-xs text-gray-500 mb-0.5">Your Copy</p>
                    <p className={`text-sm font-bold ${custSent ? 'text-green-600' : 'text-orange-500'}`}>
            ...
            <div className="bg-navy-50 rounded-xl p-4 mb-5 border border-navy-100">
              <div className="flex justify-between items-center mb-3">
                <p className="font-mono text-xs text-navy-600 font-bold">ORDER {order.orderId}</p>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-green-600 text-sm">{order.customer?.whatsapp}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs">Payment</p>
                  <p className="font-semibold text-gray-700">{order.paymentMode}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-xs">Total</p>
                  <p className="font-bold text-navy-700 text-base">₹{order.total?.toFixed(2)}</p>
                </div>
            ...
            {/* Items */}
            <h3 className="font-display font-semibold text-navy-800 mb-3">Items Ordered</h3>
            <div className="space-y-2 mb-6">
              {order.items?.map((item, i) => (
                <div key={i} className="flex justify-between items-start py-2 border-b border-gray-100 last:border-0 gap-3">
                  <div className="min-w-0">
                    <p className="font-medium text-gray-700 text-sm line-clamp-1">{item.name}</p>
                    <p className="text-xs text-gray-400">{item.itemCode} · {item.quantity} PAC × ₹{item.price}</p>
                  </div>
                  <p className="font-semibold text-navy-700 text-sm flex-shrink-0">₹{item.amount?.toLocaleString('en-IN')}</p>
                </div>
              ))}
              <div className="flex justify-between pt-2 font-bold">
                <span className="text-navy-800">Total</span>
                <span className="text-navy-700">₹{order.total?.toFixed(2)}</span>
              </div>
            </div>

            <button onClick={() => navigate('/')} className="w-full btn-primary text-center">
              🛍️ Continue Shopping
            </button>
          </div>
        </div>

...
function ProductCarousel() {
  ...
  useEffect(() => {
    // Load New Arrivals collection first, then fetch products in it
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {purchasePrice && (
                <span className="text-white/50 line-through text-sm">₹{purchasePrice.toLocaleString('en-IN')}</span>
              )}
          ...
          {slides.length > 1 && (
            <>
              <button onClick={e => handleNav(-1, e)}
            ...
            {slides.map((_, i) => (
              <button key={i} onClick={() => { clearInterval(timerRef.current); goTo(i, slides.length); }}
                className={`rounded-full transition-all duration-300 ${
                  i === index ? 'w-6 sm:w-8 h-2 bg-champagne-400' : 'w-2 h-2 bg-white/30 hover:bg-white/50'
                }`} />
            ))}
...
/* ─────────────────────────────────────────────
   MAIN HOME PAGE  (structure unchanged)
───────────────────────────────────────────── */
export default function HomePage() {
  ...
  const handleCollection = (id) => {
    setActiveCollection(id); setCurrentPage(1);
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <div className="animate-float mb-6 sm:mb-8">
            <img src="/logo.png" alt="Adiyogi International"
          ...
          <div className="page-enter">
            ...
            <div className="flex flex-col xs:flex-row gap-3 sm:gap-4 justify-center px-4 xs:px-0">
              <a href="#products" className="inline-flex items-center justify-center gap-2 bg-champagne-500 hover:bg-champagne-600 text-white font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl text-sm sm:text-lg">
                Shop Now
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                </svg>
              </a>
              <a href="#collections" className="inline-flex items-center justify-center gap-2 border-2 border-white/30 text-white hover:bg-white/10 font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-2xl transition-all duration-300 text-sm sm:text-lg backdrop-blur-sm">
                View Collections
              </a>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {collections.length > 0 ? (
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
              ...
              {collections.map(col => (
                <CollectionCard key={col._id} name={col.name} image={col.image}
                  active={activeCollection === col._id} onClick={() => handleCollection(col._id)} />
              ))}
            </div>
          ) : (
              ...
              {Array(8).fill(0).map((_, i) => (
                <div key={i} className="bg-white rounded-2xl overflow-hidden animate-pulse">
                  ...
                  <div className="p-3 sm:p-4 space-y-2 sm:space-y-3">
                    <div className="h-2.5 bg-gray-200 rounded w-1/3" /><div className="h-3.5 bg-gray-200 rounded w-full" />
                    <div className="h-5 bg-gray-200 rounded w-1/2" /><div className="h-9 bg-gray-200 rounded" />
                  </div>
                </div>
              ))}
            </div>
...
function CollectionCard({ name, image, active, onClick }) {
  // Generate a subtle gradient for cards without images
  const gradients = [
